                        ...
                        <div class="news-block row col-sm-12">
                            ...
                            <div class="row col-sm-12">
                                <div class="news-category col-sm-4">
                                    categoty: <a href="{{ route('news.category', ['id' => $news->id]) }}">{{$news->category->name}}</a>
                                </div>
                                <div class="news-visit-count col-sm-2">
                                    visited: {{ $news->visit_count }}
                                </div>
                                <div class="news-favorite-block col-sm-6">
                                    @if( Auth::check() )
                                        @if ($news->isFavorite())
                                            <a class="remove-favorite-button" href="{{ route('news.remove_favorite', ['id' => $news->id]) }}">remove from favourite</a>
                                        @else
                                            <a class="add-favorite-button" href="{{ route('news.add_favorite', ['id' => $news->id]) }}">add to favourite</a>
                                        @endif
                                    @else
